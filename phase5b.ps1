...
# 5. Сбор всех ошибок Godot (1-3 минуты, идёт реимпорт)
Write-Host "Сканирую проект Godot..." -ForegroundColor Cyan
& $godot --headless --editor --quit --path . 2>&1 | Out-File godot_errors.txt -Encoding utf8
Write-Host "=== ОШИБКИ ===" -ForegroundColor Yellow
Select-String -Path godot_errors.txt -Pattern "ERROR" | Select-Object -First 40 | ForEach-Object { $_.Line }
