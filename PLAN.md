...
4. **i18n-контент: RU и EN на 100%, 11 остальных языков — нет.**
   ...
   **Точный пересчёт на текущий HEAD (817 ключей в en.json)**:
   ...
   Итого **~3424 строки** нуждаются в реальном переводе (из 8987
   возможных = 817×11, т.е. ~38%). Крупнейшие семейства ключей:
   `SCR_*` (122 — лор/подсказки), `Q_*`/`QUEST_*` (51), `SKILL_*` (28),
   `ACH_*` (22), `END_*`/`ENDING_*` (25). Не доверять старым оценкам из
   `docs/HANDOFF.md` (там «1,562» — устарело, минимум 3 волны с тех пор
   добавляли ключи без обновления итоговой цифры). Перевод — контентная
   задача, не однострочный баг.
