...
4. **i18n-контент: RU и EN на 100%, 11 остальных языков — нет.**
   ...
   `tr()`-бага не осталось, и **нет romanized-placeholder текста** типа
   старого zh-pinyin бага — перепроверено отдельным детектором, 0 находок).
   **Точный пересчёт на текущий HEAD (817 ключей в en.json)**:

   **Обновлено 2026-09-08 (автономная волна, после SCR_*/Q_*/SKILL_*/ACH_*):**

   | Locale | Строк = английскому (backlog) |
   |---|---|
   | fr | 133 |
   | de | 124 |
   | it | 123 |
   ...
   Итого **~1215 строк** осталось (было 3424 на начало этой волны, было
   ~2191 после первого SCR_* батча). Готово: `SCR_*` (122), `Q_*` (40),
   `SKILL_*` (28), `ACH_*` (22) — все ×11 локалей, гейты зелёные каждый
   раз. Осталось крупными семействами: `QUEST_*` (11), `END_*`/
   `ENDING_*` (25), `MAP_*`/`WEAKSPOT_*`/`UPG_*` (22), плюс мелкие (см.
   разбор в коммитах). Часть оставшихся "identical" строк — легитимные
   когнаты (Auto/Park/Normal/AUDIO и т.п., совпадающие с английским по
   написанию в этих языках), не реальный пробел — не пытаться
   "исправить" искусственным синонимом. Перевод — контентная задача.
...
- [x] Перевести i18n-backlog (частично, продолжается): `SCR_*` (`d329308`),
      `Q_*` (`98da3e1`), `SKILL_*` (`88bd6a0`), `ACH_*` (`4ce4f69`) — все
      122+40+28+22 ключа × 11 языков. Backlog 3424 → 1215. Осталось:
      `QUEST_*` (11), `END_*`/`ENDING_*` (25), `MAP_*`/`WEAKSPOT_*`/
      `UPG_*` и мельче — продолжать в следующих коммитах автономной волны.
