...
2. **`emissive_windows.gd` никогда не рендерил ни одного окна, ни в
   одном районе.** Корень найден (`world_bootstrap.gd` ставит
   `EmissiveWindows` соседом `StreetBuilder`/`Props`, а не их родителем,
   поэтому `populate()`'s `_collect_walls()` всегда получает пустой
   список). **Не исправлено** — нужно сначала решить, какие меши
   реально считать «стеной» (сейчас всё, кроме `!= "Ground"`, прошло бы
   фильтр — скамейки/столбы/дорожные плиты тоже). Дизайнерское решение,
   не одна строчка кода. Файлы: `scripts/world/emissive_windows.gd`,
...
- [ ] Что должна делать 4-я ветка скилл-дерева? (Б.3)
- [ ] Какие меши считать «стеной» для окон? Или отложить фичу вообще?
      (Б.2)
- [ ] Судьба lobby/save-slots экранов: подключить или архивировать? (Б.7)
...
- [ ] Реализовать 4-ю ветку скилл-дерева (если решено делать).
- [ ] Реализовать эмиссивные окна с правильным фильтром стен (если
      решено делать).
