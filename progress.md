...
## 2026-09-13 — STORE FINAL SYNC (pass: store-sync, RC FINAL v2)
- `store/listing.md`: claim audit vs PLAYER_VISIBLE_CHANGES + PLAN + final_gate_report — 25/25 backed; deleted "free updates" promise (EN/RU + 11 locales) → honest day-one-complete closer; LAN-multiplayer data-safety line removed (archived prototype, unreachable); ads line rewritten to stub truth; +daily challenges/achievements/NG+ & save export/import lines in all 13 locales.
- ASO: flashlight/blackout/survival-puzzle/story-driven/offline woven per locale (density 1–9 hits, cap script-enforced); shorts re-labeled to true lengths (EN 70/80, RU 66/80); `gen_store_listing_locales.py --check` GREEN. NOTE: the generator tool itself is stale vs listing (CODE-owned resync recommended — do not run it without `--check`).
- `store/screenshot-plan-detailed.md` §0.5: 8/8 gallery slots already on disk in `store/screenshots/`; trailer masters marked 🎨 art-direct with exact filenames; shots 3/4/6 + RU set marked ⛔ in-game capture. `store/press-kit.md`: 31 achievements, 3–6 h, gate numbers from `docs/artifacts/final_gate_report.md`; `store/changelog.md` v1.0 final; `store/privacy-policy-template.md`: §4 LAN claim removed (EN+RU), placeholders counted (4).
- New: `docs/CERT_STORESYNC.md` (cross-table, 13/13 parity, 0 defects) + `docs/artifacts/store-sync/` verify scripts & outputs. No engine run (NEVER-GODOT). i18n en/ru masters byte-untouched (git clean).
