...
_candidates+=(
  "$CACHE_DIR/godot4"
  "$CACHE_DIR/godot"
  # Sandbox toolchain layout (from arena/01a09aec-igra's resolver).
  "$HOME/godot/godot-toolchain/Godot_v4.7-stable_linux.x86_64"
  "$HOME/godot/godot-toolchain/godot"
...
# Map host arch to the arch suffix Godot uses in its release asset names.
case "$(uname -m)" in
  x86_64|amd64)  _arch="x86_64" ;;
  aarch64|arm64) _arch="arm64" ;;
  armv7l|armv7)  _arch="arm32" ;;
  i386|i686)     _arch="x86_32" ;;
  *)             _arch="x86_64" ;;
esac

# 4.7 first: project.godot declares config/features=("4.7", ...), so an
# older engine may refuse to open the project. Older versions are kept
# only as a last-resort fallback.
#
# Asset naming note: modern Godot releases publish
# `Godot_v<ver>-stable_linux.<arch>.zip`. There is no separate
# `_linux_headless.64.zip` asset (that name was Godot 3-era and 404s on
# every 4.x release) — the standard Linux binary supports --headless.
for _ver in 4.7 4.6 4.5 4.4 4.3; do
  for _url in \
    "https://github.com/godotengine/godot/releases/download/${_ver}-stable/Godot_v${_ver}-stable_linux.${_arch}.zip" \
    "https://downloads.tuxfamily.org/godotengine/${_ver}/Godot_v${_ver}-stable_linux.${_arch}.zip"; do
    _probe "$_url" || continue
    _zip="$CACHE_DIR/godot-${_ver}.zip"
    if _download "$_url" "$_zip"; then
      if command -v unzip >/dev/null 2>&1; then
        unzip -o "$_zip" -d "$CACHE_DIR" >/dev/null 2>&1
        for _bin in "$CACHE_DIR"/Godot_v*_linux."${_arch}" "$CACHE_DIR"/Godot_v*_linux_headless.64; do
          if [[ -f "$_bin" ]]; then
