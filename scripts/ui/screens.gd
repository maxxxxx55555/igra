extends CanvasLayer
## Экраны, которых нет в UIManager: головоломки, магазин, радио, финальная ночь.
##
## ВНИМАНИЕ ПРО ЛИНТЕР: файл активно использует многострочные лямбды
## (`connect(func() -> void:` с телом на следующих строках). gdtoolkit
## (gdparse/gdlint) такой синтаксис не поддерживает и выдаёт «Unexpected
## token» — это ограничение инструмента, а не ошибка: в Godot 4 конструкция
## валидна. Синтаксис этого файла проверяется только самим движком
## (scenes/tools/compile_gate_scene.tscn).
