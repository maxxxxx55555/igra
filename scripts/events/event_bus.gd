...
##
## Правила, которые уже приходилось чинить и лучше не нарушать:
##
## 1. У сигнала должен быть слушатель. Сигнал без подписчика не даёт ни
##    ошибки, ни предупреждения — просто ничего не происходит. Так молча
##    ломались смерть игрока (game_over) и всплывающие подсказки
##    (toast_requested). Критический путь закрыт проверкой в
##    tools/flow_check.py.
## 2. Обработчик обязан принимать не меньше аргументов, чем шлёт сигнал,
##    иначе КАЖДЫЙ emit печатает ошибку, а обработчик молчит.
## 3. Подписываться на игровые события должны живые узлы 3D-сцены.
##    Слушатель внутри legacy-скриптов 2D-версии равносилен его отсутствию:
##    те узлы в дерево игры не попадают. Так не работали аптечки и батарейки.
