...
| X18 | i18n: zero missing keys / zero mixed-script / <=1.6x length across 13 locales | `data/i18n/*.json` | static (`tools/qa_sim/i18n_truth_gate.py`, blocking) + `overflow_check.py` (site-tracing, reporting-only — not wired as a gate, always exits 0, see `docs/RUN_STATE.md`) | **PARTIAL — 4/12 non-English locales PASS** on the blocking gate, 0 missing keys, 0 mixed-script. New this pass: `overflow_check.py` traced every flagged key to its real UI usage site and found 0/49 land in a fixed-width control — all are in autowrap-safe containers. Promising, but NOT a green-light on its own (reporting tool, not a verified render) — I18N-FINAL is the phase that formally reconciles this against the blocking gate, not this note |
...
- **Grand total: 113 rows.** WORKS: 57 (+46 this P2 pass: all 32 input actions via the
  ...
  pass; AL07 PowerGrid/AL21 ItemDatabase/AL22 InventoryManager/X23 win-path Endings via
  `craft_check_scene.tscn`, another unwired-but-built probe, which also surfaced and fixed a
