...
5. ✅ Done (2026-09-14): the 512² twins are wired into
   `assets/textures/cards/` — the seven duplicate-photo textures replaced
   with the unique twins, `_locked_512` variants regenerated by
   `scripts/make_locked_cards.py`, contact sheet rebuilt by
   `scripts/make_card_contact_sheet.py`. No GDScript/scene change was
   needed: `scripts/ui/collection_ui.gd` already loads cards by name
   (`res://assets/textures/cards/card_<district>[_locked]_512.png`).
   Final audit of the shipped set: min cross Hamming 13 (floor 8),
   k-means 11 singletons.
6. ⏳ Remaining (next art pass): regenerate the residential keeper card
   to close the last frozen twin (§7).
