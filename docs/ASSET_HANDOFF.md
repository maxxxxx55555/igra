...
## GOLD MASTER v4 — mobile-art pass (recreated, Path B; wired this session)

`arena/01a08c53-igra` carried no new commits (tip already an ancestor of `main`), so this
set was generated by `tools/gen_mobile_art_pass.py` and wired directly — no handoff gap.

| File / set | Size | Consumer (already wired) |
|---|---|---|
| `assets/textures/touch/touch_joy_base_256.png` + `touch_joy_knob_256.png` | 2.0/1.4 KB | `scripts/ui/virtual_joystick.gd` `_draw()` — replaces the hand-drawn placeholder circle/knob. |
| `assets/textures/touch/touch_interact_128.png`, `touch_pause_128.png` | 0.6/0.3 KB | Not yet wired into `BtnInteract`/`BtnPause` — both already carry a shipped SVG icon from the existing UI icon set; swapping without a windowed visual check risked a style mismatch. Available as an alt skin for a future pass. |
| `assets/textures/touch/touch_back_128.png` | 0.5 KB | Reserved for the Help/Codex screen back button (STEP 2 of this pass). |
| `assets/textures/touch/help_controls_96.png`, `help_battery_96.png`, `help_puzzle_96.png`, `help_stealth_96.png` | 0.3-0.7 KB | Help/Codex screen sections (controls, battery/flashlight, cable-puzzle, stealth) — STEP 2 of this pass. |
| `assets/textures/onboard_v2/onboard_05_light_cone_1024x576.png`, `onboard_06_cable_box_1024x576.png`, `onboard_07_crouch_hunter_1024x576.png` | 3.5-4.2 KB | `scripts/ui/onboarding_overlay.gd` `PANELS` — panels 5-7, i18n keys `ONBOARD_05/06/07_CAPTION` (13 locales). |

Also this pass: `scripts/systems/settings_manager.gd` gained `touch_sensitivity` /
`haptics` / `invert_look`; `scripts/ui/settings_screen.gd` Controls tab gained 3 rows;
`scripts/player/player_3d.gd` reads `invert_look` (pitch only) and `touch_sensitivity`
(touch drag-look). New headless gate: `scenes/tools/touch_probe_scene.tscn`.
