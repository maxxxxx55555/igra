...
`gas_station`). Method: python — JSON parse, regex id extraction from `data/items/*.tres`
and `data/districts/*.tres`, transitive `powered_by` closure, glob path existence,
...
Totals: 48 notes, 53 zones, 53 fixed spawns, 56 world_refs, 96 `LORE_*` i18n keys
(6 districts × 16). Every `item` / `item_type` value in every pack resolves to one of the
41 ids in `data/items/*.tres`; no pack invents an item id.

## 2. Global id checks

| Check | Result |
|---|---|
| Note ids unique across all districts (48 ids) | ✔ no collisions |
| Fixed-spawn ids unique across all districts (53 ids) | ✔ no collisions |
| Zone ids unique **within** each district | ✔ |
| Zone ids reused **across** districts | `z_exit_east` (residential, park, gas_station), `z_exit_north` (suburbs, school), `z_boiler_room` (residential, school) → **legal by design**, contract now written down (§3.3) |
| Zone ids identical across the three files of a pack | ✔ all 6 packs |
| Note ids referenced by their own `prop_manifest.md` | ✔ all 6 packs |
| Fixed-spawn ids referenced by their own `prop_manifest.md` | ✔ all 6 packs |
| Container in every fixed spawn declared in `container_modifiers` | ✔ all 6 packs |
| `world_refs` resolve to an id in `content/world/*` or `content/lore/*` | ✔ 56/56 |
| Referenced prop scenes exist (`scenes/props/*.tscn`) | ✔ |
| Referenced audio files exist | ✔ except 3 documented spec-only files: `school_lit.ogg`, `school_pipe_whisper.ogg`, `gas_station_lit.ogg` (+ optional `hospital_ward_curtain_drag.ogg`) |
| Referenced textures exist | ✔ (all 10 textures added by this PR shipped) |
...
- Act I Keeper/radio ids are used where they are guaranteed: park (their home) and
  gas_station (park is its prerequisite). ✔
...
2. Districts 1–3 use `min_stage` 0–2 only; districts 4–6 also use 3 (a FULL-gated note as
...
3. Two data-side facts flagged repeatedly by the district handoffs and still open (CODE's
   ...
   `powered_by` graph (nothing lists them as a prerequisite), and `district_themes.gd`
   `"music"` rows disagree with `music_manager.gd` for school/hospital/gas_station.
4. Audio: three lit beds specced but not fabricated (`school_lit`, `gas_station_lit`, plus
   the earlier `residential_lit`/`park_lit`); no binary audio has ever been faked by this
