...
**Статус (волна FPS-слоя, 2026-09-07).** Слой подключён к живому геймплею:
`WeaponManager` — узел игрока (`scenes/player/player_3d.tscn`), дети — три
ствола; пистолет открыт сразу, автомат/дробовик приносят `weapon_pickup`-и
(полиция/склады, см. `district_loot.gd`). Управление: огонь — ПКМ (action
`shoot`) или тач-кнопка `BtnShoot`, перезарядка — `R` / `BtnReload`, смена
ствола — колесо мыши (`weapon_next`/`weapon_prev`) или `BtnSwapWeapon`.
Хитскан — `RayCast3D` из сцены оружия (`WeaponBase.hitscan_ray`), физических
пуль нет; маска лучей = 3 (мир + монстры на 2-м слое). Резерв патронов —
предмет `ammo` в инвентаре (GDD §17), перезарядка расходует его и потому
переживает сохранение; найденные стволы пишутся в сейв
(`SaveSystem._read_weapons`). Выстрел шумит на 14 м (GDD §3.10).
...
| 3.6 | Прицел оружия (меняет цвет по врагу/цели) | `[M1]` | Реализован: луч из камеры, `enemy`/`default` (`player_3d._update_crosshair_state`) |
...
| 3.13 | Информация об оружии (режим/тип патрона/`12/36`) | `[M1]` | Реализован: счётчик `магазин / запас` (`hud_3d._on_ammo_changed`), сравнение при смене ствола |
